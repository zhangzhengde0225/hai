...
+ 2026.06.30 v1.4.8 轻量化默认安装依赖，拆分worker、mcp、legacy、ai、dev、full等可选依赖组，并将`hai`命令入口改为轻量代理。
...
+ 2026.05.17 v1.4.3 LLMWorkerV2支持了worker的版本信息展示和上报。
+ 2026.05.09 v1.4.2.1 init_register强制加载yolo等模型也改为惰性加载了。
+ 2026.05.09 v1.4.2  UAII 惰性初始化，解决打包加载依赖报错及速度慢的问题。
...
+ 2026.04.10 v1.3.8 开发了LLMWorkerV2.py，用React重新实现了worker监控和管理的前端界面。
