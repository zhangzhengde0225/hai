...
from hepai import HRModel, HModelConfig

import aiobotocore.session
from botocore.config import Config as BotoConfig
from utils import region_name, aws_access_key_id, aws_secret_access_key, proxy_definitions
...
class LLMRemoteModelV2(HRModel):
    ...
    async def anthropic_messages(self, *args, **kwargs) -> Union[Dict, AsyncGenerator]:
        ...
        # 2. 跨客户端兼容规整（messages / tools / tool_choice / 字段过滤）
        ...
        # # 4. thinking / reasoning（可能上浮 max_tokens）
        # thinking, max_tokens = self._resolve_thinking(kwargs, max_tokens)

        # 5. 组装 payload，剩余 kwargs 视为 anthropic 合法字段透传
        payload: dict = {
        ...
        if stream:
            async def stream_response():
                ...
                async with session.create_client(
                    ...
                    response = await aioclient.invoke_model_with_response_stream(
                        modelId=self.engine,
            ...
            async with session.create_client(
                ...
                response = await aioclient.invoke_model(
                    modelId=self.engine,
                    body=json.dumps(payload),
                    contentType="application/json",
                    accept="application/json",
    ...
    async def anthropic_count_tokens(self, *args, **kwargs) -> Dict:
        ...
        async with session.create_client(
            ...
            response = await aioclient.count_tokens(
                modelId=self.engine,
                body=json.dumps(payload),
            )
            print("Raw response:", response["body"])
