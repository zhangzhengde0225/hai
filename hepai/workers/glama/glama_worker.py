...
async def test_model(model_config, models: List[LLMRemoteModel]):
    ...
    llm: LLMRemoteModel = LLMRemoteModel(config=ZhizzModelConfig(engine=engine))
...
class ZhizzModelConfig(LLMModelConfig):
    ...
    base_url: str = field(default="https://glama.ai/api/gateway/openai/v1", metadata={"help": "Base url of the zhizengzeng API"})
    _api_key: str = field(default="os.environ/GLAMA_API_KEY", metadata={"help": "API key of the model"})
...
class ZhizzWorkerConfig(HWorkerConfig):
    ...
    port: int = field(default=42502, metadata={"help": "Worker's port, default is None, which means auto start from `auto_start_port`"})
