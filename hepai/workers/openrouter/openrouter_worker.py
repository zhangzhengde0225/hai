...
from hepai.tools.llm_tester import LLMConenctivityEvaluator
...
@dataclass
class ModelConfig(LLMModelConfig):
    config_file: Optional[str] = field(default=f"{here}/model_config.json", metadata={"help": "Path to the model configuration file, if None, load all models from the API"})
    base_url: str = field(default="https://openrouter.ai/api/", metadata={"help": "Base url of the zhizengzeng API"})
    _api_key: str = field(default="os.environ/OPENROUTER_API_KEY", metadata={"help": "API key of the model"})
    name: str = field(default="anthropic/claude-sonnet-4.6", metadata={"help": "Model's name"})
    engine: str = field(default="anthropic/claude-sonnet-4.6", metadata={"help": "Model engine"})
...
if __name__ == "__main__":
    ...
    models: List[LLMRemoteModel] = LLMRemoteModel.from_config(model_config.config_file)
